...
```bash
python -m jarvis.main            # full voice: speak to him, he speaks back
...
Say **"goodbye Jarvis"** (or type `exit`) to shut down.
...
jarvis/
  ...
  brain.py       # Claude agentic loop (streaming + tool use)
  ears.py        # mic capture + VAD + whisper STT
  voice.py       # Fish Audio TTS with sentence-streaming playback
